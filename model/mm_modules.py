...
class TransformerEncoder(torch.nn.Module):
    def __init__(self, n_position, d_model, n_heads, dropout, n_layers):
...
class UserEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = TransformerEncoder(
            n_position=args.max_seq_len, d_model=args.embedding_dim,
            n_heads=args.num_attention_heads, dropout=args.drop_rate,
    ...
    def forward(self, input_embs, log_mask):
        ...
        # Ensures a lower triangular mask, restricting attention to past interactions only (causal masking).
        att_mask = torch.tril(att_mask.expand((-1, -1, log_mask.size(-1), -1))).to(device)
        # Converts the mask into attention weights
        att_mask = torch.where(att_mask, 0., -1e9)
        return self.encoder(input_embs, log_mask, att_mask)
